...
`vector-rel-budget-dock` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for budget scenarios with bounded scenario files, conflict explanations, and single-node deterministic mode.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 5.
